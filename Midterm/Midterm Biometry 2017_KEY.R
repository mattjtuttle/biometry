...
hist(leaf.thickness) #Uh oh
library(fBasics)
normalTest(leaf.thickness,method = "da") #guess its okay

hist(leaf.glucosinolate)

normalTest(leaf.glucosinolate)

modFULL <- lm(LAR~leaf.glucosinolate+leaf.thickness+isothiocyanates)
vif(modFULL) #Looks good
step(modFULL)
mod <- lm(LAR~leaf.thickness+isothiocyanates) #Best Mod
summary(mod)

#B. Based on your model, what is the most important predictor for leaf area removed (and on what do you base this)?
library(QuantPsyc)
lm.beta(mod)
#Isothiocynates have biggest effect on LAR
